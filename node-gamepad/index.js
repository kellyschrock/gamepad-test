...
function stateDifference(old, curr) {
    const len = Math.min(old.length, curr.length);
    const data = [];

    for(let i = 0; i < len; ++i) {
        if(old[i] !== curr[i]) {
            data.push({ pin: i, value: curr[i]/* , old: old[i] */ });
        }
    }
...
Gamepad.prototype.emitRaw = function(emit) {
    this._emitRaw = emit;
}
...
Gamepad.prototype._maybeEmitRaw = function (data) {
    if (this._emitRaw) {
        // Get the difference between current and last state, and emit the differences.
        const raw = data;
        const diff = stateDifference(this._rawState, raw);
        if (diff) {
            this.emit("event", { data: diff });
        }
        this._rawState = data;
    }
}
